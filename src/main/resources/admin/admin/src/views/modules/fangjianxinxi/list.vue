<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        >
          <el-form-item :label="contents.inputTitle == 1 ? '民宿名称' : ''">
            <el-input
              ...
              v-model="searchForm.minsumingcheng"
              placeholder="民宿名称"
              clearable
            ></el-input>
            <el-input
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '房间类型' : ''">
            <el-input
              v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1"
              prefix-icon="el-icon-search"
              v-model="searchForm.fangjianleixing"
              placeholder="房间类型"
              clearable
            ></el-input>
            <el-input
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '房间状态' : ''">
            ...
            >
              <el-option
                v-for="(item, index) in fangjianxinxiStatusOptions"
                v-bind:key="index"
                :label="item.label"
                :value="item.value"
              >
        ...
        >
          <el-form-item>
            ...
            <el-button
              v-if="
                isAuth('fangjianxinxi', '新增') && contents.btnAdAllIcon == 0
              "
              type="success"
              @click="addOrUpdateHandler()"
              >{{ contents.btnAdAllFont == 1 ? '新增' : '' }}</el-button
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '查看') && contents.tableBtnIcon == 0
                "
                type="success"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id, 'info')"
              ...
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '通过审核') && scope.row.status == 0
                "
                type="success"
                icon="el-icon-check"
                size="mini"
                @click="approveHandler(scope.row.id)"
                >{{ contents.tableBtnFont == 1 ? '通过审核' : '' }}</el-button
              >
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '预定') &&
                    contents.tableBtnIcon == 1 &&
                    contents.tableBtnIconPosition == 1
                "
                type="success"
                ...
                size="mini"
                @click="
                  fangjianyudingCrossAddOrUpdateHandler(scope.row, 'cross')
                "
                >{{ contents.tableBtnFont == 1 ? '预定' : '' }}</el-button
              >
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '预定') &&
                ...
                @click="
                  fangjianyudingCrossAddOrUpdateHandler(scope.row, 'cross')
                "
                >{{ contents.tableBtnFont == 1 ? '预定' : ''
                }}<i class="el-icon-tickets el-icon--right"
              /></el-button>
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '预定') && contents.tableBtnIcon == 0
                "
                type="success"
                size="mini"
                @click="
              ...
              <el-button
                ...
                size="mini"
                @click="
                  fangjiantuidingCrossAddOrUpdateHandler(scope.row, 'cross')
                "
                >{{ contents.tableBtnFont == 1 ? '退订' : '' }}</el-button
              >
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '退订') &&
                ...
                @click="
                  fangjiantuidingCrossAddOrUpdateHandler(scope.row, 'cross')
                "
                >{{ contents.tableBtnFont == 1 ? '退订' : ''
                }}<i class="el-icon-tickets el-icon--right"
              /></el-button>
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '退订') && contents.tableBtnIcon == 0
                "
                type="success"
                size="mini"
                @click="
              ...
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '修改') && contents.tableBtnIcon == 0
                "
                type="primary"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id)"
              ...
              <el-button
                v-if="
                  isAuth('fangjianxinxi', '删除') && contents.tableBtnIcon == 0
                "
                type="danger"
                size="mini"
                @click="deleteHandler(scope.row.id)"
    ...
    <fangjianyuding-cross-add-or-update
      v-if="fangjianyudingCrossAddOrUpdateFlag"
      :parent="this"
      ref="fangjianyudingCrossaddOrUpdate"
    ></fangjianyuding-cross-add-or-update>
    <fangjiantuiding-cross-add-or-update
      v-if="fangjiantuidingCrossAddOrUpdateFlag"
      :parent="this"
      ref="fangjiantuidingCrossaddOrUpdate"
    ></fangjiantuiding-cross-add-or-update>
  </div>
</template>
<script>
...
import fangjiantuidingCrossAddOrUpdate from '../fangjiantuiding/add-or-update'
export default {
  data() {
    return {
      fangjianleixingOptions: [],
      fangjianxinxiStatusOptions: [
        {
          label: '待审核',
          value: 0
        },
      ...
      fangjiantuidingCrossAddOrUpdateFlag: false,
      contents: {
  ...
  components: {
    ...
    fangjiantuidingCrossAddOrUpdate
  },
  methods: {
    ...
    fangjiantuidingCrossAddOrUpdateHandler(row, type) {
      this.showFlag = false
      this.addOrUpdateFlag = false
      this.fangjiantuidingCrossAddOrUpdateFlag = true
      this.$storage.set('crossObj', row)
      this.$storage.set('crossTable', 'fangjianxinxi')
      this.$nextTick(() => {
        this.$refs.fangjiantuidingCrossaddOrUpdate.init(row.id, type)
      })
    },
    init() {
      this.$http({
    ...
    // 审核
    approveHandler(id) {
      this.$confirm(`确定进行[审核通过]操作?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$http({
          url: 'fangjianxinxi/approve/' + id,
          method: 'get'
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message({
